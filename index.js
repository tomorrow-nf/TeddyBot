  ...
  try {
    ...
    if (
      ...
      if (
        ...
        message.delete();
        console.log(
          `Deleted non-image post from #gallery from ${message.author}`
        );
      }
    }
      ...
      if (introductionsChannel != 0) {
        ...
        if (introMessages.length > 0) {
          await introductionsChannel.send(
            `${introMessages[ran]} ${member} ! Be sure to read through ${rulesChannel}.`
          );
        } else {
          await introductionsChannel.send(
