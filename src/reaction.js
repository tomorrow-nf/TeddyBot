...
import {
  handleReactionAdd as handleStarAdd,
  handleReactionRemove as handleStarRemove,
} from "./stars";
...
async function handleReactionAdd(reaction, user, TeddyBot) {
    ...
    for (let i = 0; i < emojiSets.length; i++) {
      if (reaction.emoji.id == emojiSets[i].emote) {
        ...
        if (emojiSets[i].role == 0) {
          ...
          for (let j = 0; j < emojiSets.length; j++) {
            if (emojiSets[j].set == setNumber && emojiSets[j].role != "0") {
              await reaction.message.react(
                `${TeddyBot.emojis.cache.find(
                  (emojis) => emojis.id === emojiSets[j].emote
                )}`
              );
            }
          }
          ...
          console.log(
            "Received something other than setup react, check if valid"
          );
          ...
          if (reaction.emoji.id != emojiSets[i].emote) {
            ...
            let hasRole = misc.memberHasRole(
              guild.member(user),
              emojiSets[i].role
            );
  ...
  } else await handleStarAdd(reaction, user);
}

async function handleReactionRemove(reaction, user, TeddyBot) {
  await handleStarRemove(reaction, user);
}
