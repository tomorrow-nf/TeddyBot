...
function configure() {
  ...
  if (!fs.existsSync('./img/avatar.png')) {
    ...
    fs.copyFile('./img/avatar-default.png', './img/avatar.png', function (
      err,
      data
    ) {});
  ...
  if (!fs.existsSync('./info/todo.json')) {
    ...
    console.log(
      './info/todo.json created. this is a todolist, modified with !todo.'
    );
  }
  ...
  if (!fs.existsSync('./info/reminders.json')) {
    ...
    console.log(
      './info/reminders.json created. A list of objects with Dates and Messages, sent to the ids.reminders channel once the given date is passed.'
    );
  }
  ...
  if (!fs.existsSync('./info/challenge.json')) {
    ...
    console.log(
      './info/challenge.json created. this contains entries to the current cgcc challenge.'
    );
  }

  if (!fs.existsSync('./info/challengeConfig.json')) {
    didconfigure = true;
    fs.writeFileSync('./info/challengeConfig.json', '[]', 'utf8');
    console.log(
      './info/challengeConfig.json created. this contains configuration for the current challenge.'
    );
  }
  ...
  if (!fs.existsSync('./info/votes.json')) {
    ...
    console.log(
      './info/votes.json created. This contains votes to the current CGCC challenge.'
    );
  }
